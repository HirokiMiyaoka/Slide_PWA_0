...
* リロードした場合、今回の場合はキャッシュに乗っている古いSWのスクリプトが呼ばれ、それが再インストールされてしまうため、古いバージョンが有効化される
...
* SWってポンポンインストールされて更新されるけど、内部状態ってどうやって保持するの？
    * グローバル変数にしても再インストール時にリセットされたりするので、変数はNG
        * messageのサンプルでローカル変数浸かってるが、いつ初期化されてもおかしくない
    * DBとかCacheとか使えるので、そっちに保存
