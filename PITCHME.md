...
* 必ず1日以上古いSWはキャッシュ無視をして最新版をダウンロード・インストール
...
* ブラウザでPWA対応ページを複数開く
* ブラウザで開いたPWA対応ページをアプリとしてインストールしてさらに実行

これら場合、両方のサイトが同時に動くことになりますが、これらの制御を1つのServiceWorkerが行います。同じ制御範囲では複数のSWが立ち上がったりはしません。
これに対して立ちふさがる問題が次です。
...
* クライアントとの通信や起動・活性化に対し、SWは複数の選択肢がありうる
